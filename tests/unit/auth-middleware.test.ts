...
test("requireAuth keeps req.userRole undefined for unsupported role values", async () => {
  const { requireAuth } = await import("../../src/middleware/auth.middleware.js");

  const token = jwt.sign(
    { sub: "user-123", role: "manager", tokenUse: "access" },
    process.env.JWT_ACCESS_SECRET,
    {
      algorithm: "HS256",
      expiresIn: "5m",
...
test("optionalAuth with empty authorization header keeps auth context unset", async () => {
  const { optionalAuth } = await import("../../src/middleware/auth.middleware.js");
