...
test("parseEmergencyCreateRequest keeps body photoUri when no uploaded file exists", async () => {
  const { parseEmergencyCreateRequest } = await import(
    "../../src/modules/emergency/application/emergency.request-parser.js"
  );
  ...
  const parsed = parseEmergencyCreateRequest({
    body: {
      ...
      location: '{"lat":30.2,"lng":31.3}',
      photoUri: "/uploads/from-body.jpg",
    },
    file: null,
  });

  expect(parsed.photoUri).toBe("/uploads/from-body.jpg");
});
